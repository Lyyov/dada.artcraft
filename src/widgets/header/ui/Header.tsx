...
export const Header = () => {
    ...
    return (
        <header className="fixed z-50 w-full bg-background-secondary h-nav-height">
            <div className="container h-nav-height m-auto">

                <div
                    className={clsx(
                        'md:flex md:items-center md:justify-between md:relative md:w-full md:max-w-full md:flex-row md:h-nav-height md:top-0 md:pt-0 md:translate-x-0',
...
const HeaderItem = ({ title, url }: Omit<IContentNavMenu, 'position'>) => (<Link
    role="listitem"
    className="text-sm lg:text-lg text-foreground-secondary md:text-left leading-[theme(height.nav-height)] mr-12 text-right hover:text-accent"
    href={url}
    key={title}
>
    {title}
...
const Logo = ({ isMobile }: ILogo) => (
    <Link className={clsx(
        'absolute left-0 right-0 mx-auto z-20',
