...
export const ContactForm = <T,>({ config, initialData, agreement, mode }: IInitialStateContactForm<T>) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="mt-7 grid grid-cols-12 gap-4">
            <ContactFormFields mode={mode} data={data} setData={setData} contactFormConfig={config} />
            <div className="col-span-12">
                <span className="relative inline-flex justify-center items-center mb-2">
                    <input className={clsx(
                        "btn ",
                        mode !== 'dark' && "btn bg-background-secondary text-foreground-secondary hover:text-foreground-primary"
                    )} type="submit" value="Submit" disabled={loading} />
